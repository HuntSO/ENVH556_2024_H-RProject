...
## Purpose
The purpose of this term project for ENV H 556 is to investigate the relationship between air pollution levels and socioeconomic status (SES) in Washington State. Specifically, the study aims to assess whether concentrations of nitrogen dioxide (NO₂) are higher in census tracts characterized by low SES, as identified by the Washington State Department of Health Environmental Justice Screening Tool. By analyzing this relationship, the project seeks to provide insights into potential environmental injustices and disparities in pollution exposure, contributing to a broader understanding of how social determinants of health intersect with environmental factors.

## Scientific Importance
Nitrogen dioxide (NO₂) and Black Carbon (BC) are well-documented air pollutants linked to adverse health outcomes, including respiratory and cardiovascular diseases [@dominski_effects_2021]. By examining their concentrations in relation to SES using the Washington State Department of Health Environmental Justice Screening Tool, this research contributes to the growing body of evidence on environmental health disparities.
...
## Hypothesis
NO₂ concentrations are higher in census tracts with low socioeconomic status as defined by the Washington DOH Environmental Justice Screening Tool.
...
The selection of covariates for this study was guided by their relevance to understanding and predicting air pollution exposure in relation to socioeconomic status (SES) [@blanco_characterization_2022]. The chosen covariates reflect various geographic, demographic, and infrastructural factors that are known to influence traffic-related air pollution (TRAP).
...
Nitrogen dioxide (NO₂) levels were characterized using descriptive statistics, including mean, standard deviation, geometric mean, and measures of missing data, to assess their distributions. Data were sourced from annual pollutant measurements, mobile covariates, and census block and grid-based datasets. These datasets included key geographic covariates (e.g., proximity to roadways, truck routes, bus routes, ports, and airports) and demographic indices (e.g., population density, traffic load, and environmental justice metrics) selected based on established associations with traffic-related air pollution (TRAP).

Universal Kriging (UK), a geostatistical method incorporating spatial dependence and covariates, was used to predict pollutant levels. Spatial structure was assessed through variogram modeling, and best-fit models were identified. Predictions were generated at census block centroids and visualized on OpenStreetMap tiles to align predictions with population exposure at a local scale. Prediction uncertainties were quantified through standard error estimation.

Model performance was validated using cross-validation methods, including 5-fold cross-validation and leave-one-out cross-validation (LOO). Metrics such as root mean square error (RMSE) and R² were used to evaluate prediction accuracy.
Socioeconomic data, sourced from EPA EJSCREEN, provided demographic indices (DEMOGIDX_2 and DEMOGIDX_5) for further analysis. Relationships between NO₂ predictions and socioeconomic indices were examined using scatter plots with regression lines. Predicted pollutant levels were visualized on choropleth maps, overlaid with socioeconomic indices to explore spatial and demographic patterns

### Site Data Treatment

Differences in pollutant concentrations between collocation and normal sites were observed in the descriptive analysis. However, the number of collocation sites was too limited to justify splitting the data based on site type for kriging. Consequently, all measurements were aggregated, and predictions were made using the combined dataset to maintain sufficient spatial coverage and model robustness.
...
The map provided a clear spatial representation of the variability in NO₂ levels, with additional context from a bounding box, scale bar, and north arrow. These visualizations revealed localized patterns of higher NO₂ concentrations and established a basis for further investigation into spatial and demographic disparities (Figure 2).

**Figure 1: Seattle NO2 Grid Map**
...
**Figure 3: Annual UK Predictions to Centroids**
...
**Figure 4: NO2 & SES Indices Scatterplots**
```{r, echo = FALSE}
...
**Table 1: NO2 Model Cross Validation**
...
Rachael contributed code for analysis, data cleaning, cleaning, data preparation, drafted tables and figures, and editing of report.

Hunt contributed data gathering, drafted writings & report, and set up collaboration systems (github).

Both authors contributed equally to hypothesis generation and method development with assistance from ENV H 556 instructors.

# References

[@blanco_characterization_2022]

[@dominski_effects_2021]

# Results Appendix

**Appendix Table 1: Annual Data Descriptives**

```{r make a summary table, echo=TRUE, echo = FALSE}
#-----table with dplyr------
...
**Appendix Figure 1: NO2 Distribution Histogram**

```{r histogram of no2, warning=FALSE, message=FALSE, echo = FALSE}
...
The semi-variogram cloud illustrates how semi-variance changes with spatial distance. The red smoothed curve shows the general trend, with semi-variance increasing slightly as distance grows, indicating moderate spatial autocorrelation in NO₂ levels. Although individual points vary, the curve provides a reliable foundation for fitting a variogram model, which is crucial for Universal Kriging. The flatter trend at larger distances suggests a limited range of spatial dependence.

**Appendix Figure 2: Semi-Variogram Smoothed**

```{r empirical variogram of no2, echo=FALSE, echo = FALSE}
